...
function [total, UM, MAN] = runExperiment(lambda)
    ...
    R = (100^2)*eye(2); % measurment variance
    
    G = [0.5*(T^2); T]; % coefficient of the model noise %1D
    G = [G', 0, 0;  0, 0, G']';
...
%     RMSE_GKF = zeros(1, num_experiments);
    RMSE = @(est, traj) mean((est(1,:) - traj(1,:)).^2 + (est(2,:) - traj(2,:)).^2)^0.5;
    for per = 1 : num_experiments
...
%         legend(["gt", "meas", "kf", "imm"]);
        ...
        UM_results{per, 2} = results{per, 2}.x_posterior(UM_idx,[1,3])';
        UM_results{per, 3} = results{per, 3}.x_posterior(UM_idx,[1,3])';
        UM_results{per, 4} = results{per, 4}.x_posterior(UM_idx,[1,3])';
%         UM_results{per, 5} = results{per, 5}.x_posterior([1:12 25:36 49:60 73:84],1)';
        UM_results{per, 6} = results{per, 6}{1}(:, UM_idx);
        
        MAN_results{per, 1} = results{per, 1}.x_posterior(MAN_idx,[1,3])';
        MAN_results{per, 2} = results{per, 2}.x_posterior(MAN_idx,[1,3])';
        MAN_results{per, 3} = results{per, 3}.x_posterior(MAN_idx,[1,3])';
        MAN_results{per, 4} = results{per, 4}.x_posterior(MAN_idx,[1,3])';
%         MAN_results{per, 5} = results{per, 5}.x_posterior([13:24 37:48 61:72],1)';
        MAN_results{per, 6} = results{per, 6}{1}(:, MAN_idx);
        
        
        % Total RMSE's
        RMSE_KF1(1, per) = RMSE(results{per, 1}.x_posterior(:, [1,3])', results{per, 6}{1});
        RMSE_KF2(1, per) = RMSE(results{per, 2}.x_posterior(:, [1,3])', results{per, 6}{1});
        % RMSE_IMM(1, per) = RMSE(results{per, 3}.x_posterior(:, 1)', results{per, 5}{1}(1, :));
        RMSE_IMM(1, per) = RMSE(results{per, 3}.x_posterior(:, [1,3])', results{per, 6}{1});
        RMSE_KF(1, per) = RMSE(results{per, 4}.x_posterior(:, [1,3])', results{per, 6}{1});
%         RMSE_GKF(1, per) = RMSE(results{per, 5}.x_posterior(:, [1,3])', results{per, 6}{1});
